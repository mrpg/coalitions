...
def coalitions(parties, *, unfeasible = [], remove_extraneous = True, majority = 0.5, threshold = 0.05):
	...
	for coalition in powerset(parties): # iterate over powerset of parties
		if not coalition == (): # ignore empty set
			if all(not (u[0] in coalition and u[1] in coalition) for u in unfeasible): # ignore coalitions with enmities
				if (maj := sum(y for x, y in parties.items() if x in coalition)) > majority: # ignore coalitions that have no majority
					if remove_extraneous:
						if all(maj-y <= majority for x, y in parties.items() if x in coalition): # check whether all parties in the coalition are strictly necessary
							yield coalition, maj
					else:
						yield coalition, maj
